...
#include "lanelet2/lanelet2_core/LaneletMap.h"
...
#include "lanelet2/lanelet2_core/geometry/Area.h"
#include "lanelet2/lanelet2_core/geometry/BoundingBox.h"
#include "lanelet2/lanelet2_core/geometry/Lanelet.h"
#include "lanelet2/lanelet2_core/geometry/LineString.h"
#include "lanelet2/lanelet2_core/geometry/Polygon.h"
#include "lanelet2/lanelet2_core/geometry/RegulatoryElement.h"
...
struct PrimitiveLayer<T>::Tree {
  ...
  void erase(const T& elem) {
    TreeNode node = treeNode(elem);
    if (!node.first.isEmpty()) {
      std::cout << "remove node sucess!!!" << std::endl;
      rTree.remove(node);
    }
  }
  RTree rTree;
...
void LaneletMap::remove() {
  {
    std::cout << "remove test losgehen!" << std::endl;
    for (auto& value : this->regulatoryElementLayer) {
      lanelet::Id& id = std::const_pointer_cast<lanelet::RegulatoryElementData>(value->constData())->id;
      std::cout << "id of the regulatory Element" << id << std::endl;
      // lanelet::Id& id = value->getId();
      // this->regulatoryElementLayer.remove(id);
      this->regulatoryElementLayer.tree_->erase(value);
    }
  };
  // this->regulatoryElementLayer.tree_ = nullptr;
  this->regulatoryElementLayer.elements_.clear();
}
