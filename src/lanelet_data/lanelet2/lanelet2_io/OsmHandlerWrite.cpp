#include <lanelet2/pugixml/pugixml.hpp>
#include <sstream>
#include "lanelet2/lanelet2_io/Exceptions.h"
#include "lanelet2/lanelet2_io/io_handlers/Factory.h"
#include "lanelet2/lanelet2_io/io_handlers/OsmFile.h"
...
class ToFileWriter {
 ...
 private:
  ...
  void writeNodes(const LaneletMap& map, const Projector& projector) {
    ...
    std::cout<<"befor for map.pointLayer..."<<std::endl;
    for (const auto& point : map.pointLayer) {
      try {
        GPSPoint gpsPoint = projector.reverse(point);
        // std::cout<<"at after projector.reverse()..."<<std::endl;
        if (point.hasAttribute("gps_lon"))
        {
          gpsPoint.lon = point.attribute("gps_lon").asDouble().value();
        }
        if (point.hasAttribute("gps_lat"))
        {
          gpsPoint.lat = point.attribute("gps_lat").asDouble().value();
        }
        // std::cout<<"do gps_lon_lat"<<std::endl;
      ...
      std::cout<<"do gps_lon_lat 233"<<std::endl;
  ...
  osm::Attributes getAttributes(const AttributeMap& attributes) {
    ...
    for (const auto& attr : attributes) {
      std::string str1 = "gps_lon";
      std::string str2 = "gps_lat";
      if (attr.first == str1 || attr.first == str2)
      {
        continue;
      }
