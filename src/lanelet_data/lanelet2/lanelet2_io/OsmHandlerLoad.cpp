  ...
  void loadNodes(const lanelet::osm::Nodes& nodes, const Projector& projector) {
    for (const auto& nodeElem : nodes) {
      ...
      try {
        ...
        auto point = Point3d(node.id,projector.forward(node.point), getAttributes(node.attributes));
        ...
        {
            point.x() = point.attribute("x").asDouble().value();
        }
        ...
        {
            point.y() = point.attribute("y").asDouble().value();
        }
        points_.emplace(node.id,point);
