...
Eigen::MatrixXd InformationMatrixCalculator:: default_information_matrix() const{
  Eigen::MatrixXd inf = Eigen::MatrixXd::Identity(6, 6);
  double v_x = std::pow(max_stddev_x, -2);
  double v_q = std::pow(max_stddev_q, -2);
  inf.topLeftCorner(3, 3).array() *= v_x;
  inf.bottomRightCorner(3, 3).array() *= v_q;
  return inf;
}
